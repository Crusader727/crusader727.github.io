...
## 4A. Прогресс-бар выполнения (обязательно)

После завершения фазы **планирования** (когда задача разбита на шаги) и далее **после каждого закрытого шага** — выводи прогресс-бар: сколько сделано из плана.
...
✅ Шаг 2 — <название>
✅ Шаг 3 — <название>
🔄 Шаг 4 — <название> (в работе)
⬜ Шаг 5 — <название>
⬜ Шаг 6 — <название>
```

Правила:
- Бар из **20 ячеек**: заполнено = `round(20 × done/total)`, символы `█` (готово) и `░` (осталось).
- Проценты — целые; `done/total` считается по подзадачам **нижнего уровня** плана.
- Показывай сразу **после плана** (0/N, бар пустой) и обновляй после каждого завершённого шага; на 100% — финальный полный бар.
- Статусы шагов: `✅` готово, `🔄` в работе, `⬜` не начато.
- Мелкие задачи (1–2 файла, hotfix, рефакторинг без смены поведения) — **без фазы планирования и без бара**.
- Веди тот же список параллельно через `TaskCreate`/`TaskUpdate`; бар — его визуальное отражение в ответе.
